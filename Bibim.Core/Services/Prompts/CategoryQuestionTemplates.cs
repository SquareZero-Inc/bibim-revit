...
namespace Bibim.Core
{
    ...
    public static class CategoryQuestionTemplates
    {
        ...
        public static string BuildPlannerChecklist()
        {
            ...
            sb.AppendLine("parameter — SET/EDIT a value (변경/수정/입력/기입/설정/채워/넣어/set/input/fill — the DEFAULT for ANY parameter work): do NOT ask binding / data-type / parameter-group / instance-vs-type questions. codegen finds the parameter at runtime (LookupParameter / BuiltInParameter), edits type-level/read-only correctly, and reports honestly if it is absent. Ask only the target value or scope if genuinely missing — never a creation-style question.");
            sb.AppendLine("parameter — CREATE a NEW definition: RARE. Only when the user literally says 파라미터 만들/생성/정의 (create/define a parameter). Even then, do NOT turn binding/group/data-type into clarification questions — codegen picks sensible defaults. Setting a value is NEVER this category.");
